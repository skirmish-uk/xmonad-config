...
myFont :: String
myFont = "Menlo:SauceCodePro Nerd Font Mono:regular:size=9:antialias=true:hinting=true"
...
myStartupHook = do
  ...
  spawnOnce "xsetroot -xcf /usr/share/icons/Sweet-cursors/cursors/left_ptr 28"
  spawn "killall conky"   -- kill current conky on each restart
  spawn "killall trayer"  -- kill current trayer on each restart

  spawnOnce "lxsession"
  spawnOnce "openrazer-daemon &"
  spawnOnce "picom"
  spawnOnce "nm-applet"
  ...
  spawn ("sleep 2 && xmodmap -e 'pointer = 1 2 3'") -- openrazer decides to remap mouse buttons so here is a hotfix
  -- spawnOnce "~/.fehbg &"  -- set last saved feh wallpaper
  -- spawnOnce "feh --bg-fill /home/jordan/Pictures/Fp0Sdo6.jpeg"  -- feh set random wallpaper
  spawnOnce "xrandr -s 5120x1440 -r 120 && sleep 1 && xrandr --output HDMI-A-0 --mode 1920x1080 --rate 60 --pos 5120x640" -- xrandr set resolution and refresh rate
  spawnOnce "variety &" -- variety wallpapers
  setWMName "LG3D"
...
spawnSelected' lst = gridselect conf lst >>= flip whenJust spawn where conf = def {
    gs_cellheight   = 40, 
    gs_cellwidth    = 180, 
    gs_cellpadding  = 6, 
    gs_originFractX = 0.5, 
...
threeRow = renamed [Replace "threeRow"]
           ...
           $ Mirror
           $ ThreeCol 1 (3/100) (1/2)
multiCols = renamed [Replace "multiCol"]
           $ smartBorders
           $ windowNavigation
           $ addTabs shrinkText myTabTheme
           $ subLayout [] (smartBorders Simplest)
...
myLayoutHook = avoidStruts
  ...
  where
    myDefaultLayout = withBorder myBorderWidth multiCols
                                           ||| noBorders monocle
                                           ||| floats
                                           ||| noBorders tabs
                                           ||| grid
...
myWorkspaces = [" term ", " www ", " chat ", " games ", " vbox ", " sys ", " mus ", " vid ", " gfx "]
...
myManageHook = composeAll
  ...
  , (className =? "Discord" <&&> (title =? "" <||> resource =? "Dialog")) --> doCenterFloat
  , (className =? "Steam" <&&> resource =? "Dialog")                      --> doCenterFloat
  , (className =? "youtube-music-desktop-app" <&&> resource =? "Dialog")  --> doCenterFloat
  , title =? "Oracle VM VirtualBox Manager"                               --> doCenterFloat
  , title =? "Mozilla Firefox"                                            --> doShift ( myWorkspaces !! 1 )
  , className =? "youtube-music-desktop-app"                              --> doShift ( myWorkspaces !! 1 )
...
showKeybindings x = addName "Show Keybindings" $ io $ do
  h <- spawnPipe $ "yad --text-info --fontname=\"SauceCodePro Nerd Font Mono 12\" --fore=#dddddd back=#1a1a1a --center --geometry=1200x800 --title \"XMonad keybindings\""
...
myKeys c =
  ...
  , ("M-d", addName "Run prompt"      $ sequence_ [spawn (mySoundPlayer ++ dmenuSound), spawn "/usr/bin/dmenu_run"])
  ...
  , ("M-p i", addName "Take a screenshot"      $ spawn "dm-screenshot")
  ...
  , ("M-p q", addName "Logout Menu"            $ spawn "dm-lock")
